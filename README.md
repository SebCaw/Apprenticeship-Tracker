...
```
GitHub Actions (cron, 2×/day)  ──►  scrape.ps1  ──►  GOV.UK  ──►  data.json  ──►  GitHub Pages (tracker.html)
```
...
- **`index.html`** — the landing / About page; visitors arrive here and click through to the tracker.
- **`tracker.html`** — a self-contained dashboard (vanilla HTML/CSS/JS, no framework) that loads `data.json` and renders everything, with progress tracking persisted in `localStorage`.
